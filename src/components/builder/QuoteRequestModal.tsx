...
export function QuoteRequestModal({ open, onClose, onSubmit }: QuoteRequestModalProps) {
  const { t } = useI18n()
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    setError('')
    onSubmit(trimmedName, normalizedPhone)
    setName('')
    setPhone('')
  }
    ...
    <Modal open={open} title={t('quoteRequestTitle')} onClose={handleClose} size="md">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
          <button type="button" className="btn-ghost rounded-xl px-4 py-2.5 text-sm font-semibold" onClick={handleClose}>
            {t('cancel')}
          </button>
          <button type="submit" className="btn-primary rounded-xl px-4 py-2.5 text-sm font-semibold">
            {t('quoteRequestSubmit')}
          </button>
        </div>
      </form>
    </Modal>
