import { AlertTriangle, CheckCircle2, Cpu, GitCompare, Lock, Search, Zap } from 'lucide-react'
...
export function PCBuilderPage() {
  ...
  const categoryProducts = useMemo(
    () =>
      products.filter(
        (product) => product.category === activeCategory && product.stock > 0 && !product.discontinued,
      ),
    [products, activeCategory],
  )

  const activeProducts = useMemo(() => {
    const base = showAllParts
      ? categoryProducts
  ...
  }, [categoryProducts, build, showAllParts, slotSearch])

  const useCaseHint = inferBuildUseCaseSummary(build)

  const psuWattage = Number.parseInt(build.PSU?.specs.wattage ?? '0', 10) || 0
          ...
          {slotsOrder.map((slot) => {
            ...
            return (
              <button
                ...
                className={`w-full rounded-xl border p-3.5 text-left transition duration-200 ${
                  isActive
                ...
                } ${!unlocked ? 'cursor-not-allowed opacity-45' : 'hover:-translate-y-0.5'}`}
              >
                <div className="flex items-center justify-between gap-2">
                  <p className="font-semibold text-white">{slot}</p>
                  {!unlocked && <Lock size={14} className="text-text-muted" />}
                </div>
                <p className="mt-1 truncate text-sm text-text-muted">
                  {selected ? selected.name : unlocked ? t('selectPart') : t('locked')}
                </p>
              </button>
            )
          })}
              ...
              return (
                <div key={product.id} style={{ animationDelay: `${index * 50}ms` }}>
                  <ProductCard
                    ...
                    showCompatibilityBadge={showAllParts}
                    disabled={!isUnlocked(activeCategory)}
        ...
        <ul className="space-y-2">
          {slotsOrder.map((slot) => (
            ...
            >
              ...
              <span className="max-w-[60%] truncate text-sm text-text-muted">{build[slot]?.name ?? '—'}</span>
            </li>
          ))}
        </ul>
